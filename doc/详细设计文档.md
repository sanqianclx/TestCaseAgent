...
| `parse-source`     | `parse_source_code` Tool（Python 或 Language Adapter） | 源码内容、文件名             | AST结构、符号列表、warnings    |
| `summarize-logic`  | `test-case-agent`                | 符号信息、源码片段            | 逻辑摘要、输入约束、分支           |
| `generate-cases`   | `test-case-agent`                | AST、逻辑摘要、需求文本        | 测试用例列表                 |
| `generate-code`    | `test-code-agent`                | 用例、模块名、历史失败信息        | 对应语言测试代码（pytest / JUnit 5 / gtest） |
...
| `measure-coverage` | `coverage_tool` → Language Adapter | 执行产物（.exec / .gcda）、源文件目录 | 真实行/分支覆盖率              |
...
> 说明：V2.0 起 `check-quality` 步骤下线，对应的断言质量由 LLM 在 `generate-code` 阶段内嵌 prompt 约束；`measure-coverage` 步骤作为补充质量信号在 `execute-tests` 之后追加调用。`execute-tests` 步骤由 `Language Adapter` 统一接口分发到 python-adapter / java-adapter / cpp-adapter，产物结构一致。
...
| `testCodeAgent`        | 生成或自愈对应语言的测试代码（pytest / JUnit 5 / gtest） | `create_test_code_version`、历史失败信息 |
| `diagnosisAgent`       | 诊断测试失败根因并给出next\_action     | `context_search`、版本读取工具           |
| `cliConversationAgent` | 在 CLI 入口承接用户自由输入的指令（重新生成/调整覆盖率/调整诊断等），输出结构化决策 | 语言适配器查询、`measure_coverage` 结果 |
