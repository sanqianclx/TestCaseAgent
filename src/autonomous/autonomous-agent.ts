...
- measure-coverage：先执行测试再解析覆盖率。
  - **诚实原则**（V2.6.4）：如果返回 ok=false 且 error.code 是 NOT_IMPLEMENTED，**必须**在最终输出里**直接告诉用户"该语言覆盖率工具未实现"**。**严禁**编造覆盖率数字（"91.84%"之类）。如果工具没实现，就说没实现；如果没有真实数据，就写"覆盖率数据未获取"，不要 LLM 自己心算。
- **C++ 编译/覆盖率工具路径**（V2.6.5，重要）：机器上 PATH 里 Git Bash 的 mingw64（路径 /mingw64/bin）优先级高于 MSYS2 UCRT64，会污染 cc1plus 的 DLL 加载导致 g++ 静默失败。**C++ 编译/coverage 必须用绝对路径**：
  - g++ 路径：D:\\msys2\\ucrt64\\bin\\g++.exe（MSYS2 UCRT64 16.1.0）
  - gcov 路径：D:\\msys2\\ucrt64\\bin\\gcov.exe（同上）
  - 严禁用 g++ / gcov（会被 PATH 截胡到 Git mingw64 的 8.1.0 旧版）
  - 编译后产生的 .gcno / .gcda 文件名是 test-foo.* 形式（g++ 16.1 加了 dumpdir），gcov 默认找 foo.*——调用 gcov 前先 cp test-foo.gcno foo.gcno 然后 cp test-foo.gcda foo.gcda
